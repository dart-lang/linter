...
void main() {
  ...
  test('equality', () {
    expect(
        Annotation('Actual message (to be ignored)', ErrorType.LINT, 1,
            column: 0, length: 1),
        matchesAnnotation(null, ErrorType.LINT, 1, column: 0, length: 1));
    expect(Annotation('Message', ErrorType.LINT, 1, column: 0, length: 1),
        matchesAnnotation('Message', ErrorType.LINT, 1, column: 0, length: 1));
  });

  test('inequality', () {
    expect(
        () => expect(
            Annotation('Message', ErrorType.LINT, 1, column: 0, length: 1),
