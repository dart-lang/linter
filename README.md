...
and see any violations of the `annotate_overrides`, `hash_and_equals`, and `prefer_is_not_empty` rules in the console.  In practice you would probably configure quite a few more rules ([the full list][lints]).  The evolving list of [lints enforced internally at Google][google-lints] may also serve as inspiration.
